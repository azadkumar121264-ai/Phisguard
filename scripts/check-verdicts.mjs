...
const DETECTED = { malicious: 3, suspicious: 0, harmless: 60, undetected: 20 };
...
expectVerdict('http://goggle.com/login', NO_VERDICT, 'completed', ['MALICIOUS']);
expectVerdict('https://paypa1-login.tk/verify-account', NO_VERDICT, 'completed', ['MALICIOUS']);
expectVerdict('http://secure-hdfcbank-kyc-update.top/wp/login.php', NO_VERDICT, 'completed', ['MALICIOUS']);
expectVerdict('http://user@evil.tk/paypal/verify', NO_VERDICT, 'completed', ['MALICIOUS']);
expectVerdict('http://free-lottery-winner.xyz/claim-now', NO_VERDICT, 'completed', ['MALICIOUS']);
...
expectVerdict('http://192.168.0.1/login', null, 'skipped', ['MALICIOUS']);
...
expectVerdict('https://paypa1-login.tk/verify-account', null, 'unavailable', ['MALICIOUS']);
